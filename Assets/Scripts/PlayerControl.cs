    ...
    public class PlayerControl : MonoBehaviour
    {
        ...
        {
        if (scene.name == "Map2_1" && !GameManager.Instance.daQuaMapTruoc)
        {
            riceCount = 0;
        ...
        // Tìm SpawnPoint (không phân biệt hoa thường)
        GameObject spawnPoint = null;

            // Cách 1: Tìm theo tên chính xác
            spawnPoint = GameObject.Find("SpawnPoint");

            // Cách 2: Nếu không thấy, tìm theo Tag
            if (spawnPoint == null)
            {
                spawnPoint = GameObject.FindWithTag("SpawnPoint");
            }

            // Cách 3: Nếu vẫn không thấy, tìm kiếm toàn bộ
            if (spawnPoint == null)
            {
